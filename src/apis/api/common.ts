...
export const changeNickname = async (game: string, nickname: string) => {
  const getOverwatchNickname = (nickname: string): string => {
    const [name, battleTag] = nickname.split('#');
    return `${name}%23${battleTag}`;
  };

  const response = await authAxios.put(
    `/api/user/${game.toUpperCase()}/${
      game !== 'overwatch' ? nickname : getOverwatchNickname(nickname)
    }/nickname`,
  );
